...
class TM1637:
	...
	def Show( self, data ):
		for i in range(0,4):
			try:
				self.__currentData[i] = HexDigits[data[i]];
			except:
				self.__currentData[i] = 0;

			self.__currentData[i] = self.__currentData[i] | (0x80 * (self.__point[i] > 0))
		self.Update()
	# end  Show

	def Point( self, data ):
		for i in range(0,4):
			self.__point[i] = data[i];
			self.__currentData[i] = self.__currentData[i] & 0x7F
			self.__currentData[i] = self.__currentData[i] | (0x80 * (self.__point[i] > 0))
		self.Update()
	# end  Show
	...
	def Update(self):
		...
		for i in range(0,4):
			self.writeByte(self.__currentData[i]);
		self.stop();
		self.start();
